...
Features
--------
• N×N grid annotation — identical workflow for raster images and GPKG files
• Zoom + / − (mouse wheel, buttons, keys = / −)
• Pan inside a zoomed canvas (right-click drag OR arrow keys)
• GeoTIFF multi-band loading (via rasterio with PIL fallback)
• GPKG rasterised as background map, then annotated on the same N×N grid
• GPKG / metadata auto-annotation: cells with >75 % non-black pixels → Habitat
• 3 annotation classes:
    Habitat              (green)   key h
    Restorable Accessible(orange)  key r  — inline cost entry
    Non-Restorable       (grey)    key n  — merges old "RNA" + "NHNR"
• Multi-cell paint: hold left mouse button and drag to annotate continuously
...
Usage
-----
  python annotation_v9.py --write-config    # write conf/annotator.yaml
  python annotation_v9.py                   # file picker (uses YAML if present)
  python annotation_v9.py --image zone.tif
  python annotation_v9.py --gpkg zones.gpkg --N 30
  python annotation_v9.py layers.roads.enabled=true   # Hydra override
...
---------------
  ...
  pip install geopandas matplotlib         # GPKG annotation, vector layers
  pip install osmnx networkx scipy         # OSM roads/hydrology, accessibility
...
def write_default_yaml(config_dir: str = "conf") -> Path:
    ...
    # annotation_v9.py — Hydra configuration
    # Override from CLI: python annotation_v9.py layers.roads.enabled=true
    # ──────────────────────────────────────────────────────────────────────────
    ...
    layers:
      ...
      roads:
        enabled:   false
        path:      ""      # path to shapefile/GPKG (empty = fetch from OSM)
...
CLASS_META = {
    ...
    CLASS_RA:   dict(label="Restorable Accessible", rgb=(244, 162,  97), alpha=140, key="r"),
...
class AnnotationGrid:
    ...
    def to_arrays(self) -> dict[str, np.ndarray]:
        ...
        Returns
        -------
        habitat    : 1 = habitat present, 0 = absent
        restorable : 1 = Restorable Accessible cell, 0 otherwise
        accessible : same as restorable (only CLASS_RA is both)
        cost       : restoration cost (float; 0 where not CLASS_RA)
        """
        habitat    = (self.labels == CLASS_HAB).astype(int)
        restorable = (self.labels == CLASS_RA ).astype(int)
        #accessible = restorable.copy()
        layers = extract_layers_from_osm()
        roads = layers["roads_gdf"]
        print(roads.geometry.geom_type.value_counts())
        accessible = compute_accessibility_from_roads(polygon=poly,N=10,max_distance_m=10)
...
def _make_square_bounds_3857(
    minx: float, miny: float, maxx: float, maxy: float
) -> Tuple[float, float, float, float]:
    """Expand bounds to a square in EPSG:3857 (metre-based)."""
...
def _bounds_3857_to_4326(bounds: Tuple) -> Tuple:
    ...
    try:
        ...
        t    = Transformer.from_crs("EPSG:3857", "EPSG:31985", always_xy=True)
        lon1, lat1 = t.transform(bounds[0], bounds[1])
        lon2, lat2 = t.transform(bounds[2], bounds[3])
        return lon1, lat1, lon2, lat2
    except ImportError:
        # Rough approximation (±5 km for mid-latitudes)
        k = 1 / 111_320
...
def _render_matplotlib_to_pil(
    ...
    with axes locked to sq_bounds_3857 = (minx, miny, maxx, maxy).
 
    All layer rendering uses this helper so every layer is pixel-aligned.
    """
    import matplotlib
    matplotlib.use("Agg")
    ...
    ax.set_xlim(sq_bounds_3857[0], sq_bounds_3857[2])
    ax.set_ylim(sq_bounds_3857[1], sq_bounds_3857[3])
    ax.set_aspect("equal")
    ax.axis("off")
...
# =============================================================================
# 4.  IMAGE / GPKG LOADING  (returns bounds)
# =============================================================================
 
# Return type: (PIL RGBA, sq_bounds_3857 or None, sq_bounds_4326 or None, GDF or None)
...
def _load_tiff_with_meta(path: str, size: int) -> LoadResult:
    """Load GeoTIFF, extract square bounds, return PIL RGBA + bounds."""
    sq3 = sq4 = None
    try:
        import rasterio
        ...
        with rasterio.open(path) as ds:
            ...
            # Extract square bounds in 3857 and 4326
            try:
                raw = ds.bounds
                b3 = transform_bounds(ds.crs, "EPSG:31985",
                                      raw.left, raw.bottom, raw.right, raw.top)
                sq3 = _make_square_bounds_3857(*b3)
                sq4 = _bounds_3857_to_4326(sq3)
            except Exception:
                pass
 
        rgb = np.stack([r, g, b], axis=-1)
        return _square_crop_resize(Image.fromarray(rgb, "RGB"), size), sq3, sq4, None
...
def _load_gpkg_with_meta(path: str, layer: Optional[str],
                          size: int) -> LoadResult:
    """Rasterise a GPKG layer, extract square bounds."""
    ...
    if gdf.crs is not None and gdf.crs.is_geographic:
        ...
        gdf = gdf.set_crs("EPSG:4326").to_crs("EPSG:31985")
 
    sq3 = _make_square_bounds_3857(*gdf.total_bounds)
    sq4 = _bounds_3857_to_4326(sq3)
    ...
    def _plot(ax):
        ...
        # Feature ID labels
        extent = max(sq3[2] - sq3[0], sq3[3] - sq3[1])
    ...
    img = _render_matplotlib_to_pil(_plot, sq3, size, bg_color="#1a1a2e")
    return img, sq3, sq4, gdf
 
 
def _load_source_with_meta(source: str, layer: Optional[str],
                            size: int) -> LoadResult:
    ext = Path(source).suffix.lower()
    if ext in (".tif", ".tiff"):
        return _load_tiff_with_meta(source, size)
    elif ext in (".gpkg", ".shp", ".geojson", ".json"):
        return _load_gpkg_with_meta(source, layer, size)
    else:
...
def _render_elevation_layer(
    ...
    **No file / file not found**
    ...
    cfg : LayerItemConfig
        ...
        ``cfg.colormap`` — matplotlib colormap name (default ``"terrain"``).
    ...
    if cfg.path and Path(cfg.path).exists():
        ...
        if ext in (".gpkg", ".shp", ".geojson", ".json"):
            ...
            print( cfg.path, sq_bounds_3857, size, dn_field)
...
def _elevation_from_vector(
    ...
    if str(gdf.crs) != "EPSG:31985":
        try:
            gdf = gdf.to_crs("EPSG:31985")
        except Exception as exc:
            print(f"  [elevation/vector] CRS reprojection failed: {exc}")
            return None
    ...
    if (has_polys or has_lines) and _HAS_RASTERIO_FEATURES:
        ...
        # print("*****", shapes1[:10])
        # [({'id': '0', 'type': 'Feature', 'properties': {}, 'geometry': {'type': 'Polygon', 'coordinates': (
        #     ((-3893126.950529296, -723511.788276811), (-3893127.071429465, -723543.0722250056),
        #      (-3893096.3358754246, -723543.1925859154), (-3893096.214992151, -723511.9086324293),
        #      (-3893126.950529296, -723511.788276811)),)},
        #    'bbox': (-3893127.071429465, -723543.1925859154, -3893096.214992151, -723511.788276811)}, np.float64(6.0)),
        #  ({'id': '1', 'type': 'Feature', 'properties': {}, 'geometry': {'type': 'Polygon', 'coordinates': (
        #      ((-3893065.358588746, -723480.7450294184), (-3893065.479449808, -723512.0289712269),
        #       (-3893096.214992151, -723511.9086324293), (-3893096.3358754246, -723543.1925859154),
        #       (-3893034.8647517473, -723543.4332572696), (-3893034.623058099, -723480.865346105),
        #       (-3893065.358588746, -723480.7450294184)),)},
        #    'bbox': (-3893096.3358754246, -723543.4332572696, -3893034.623058099, -723480.7450294184)}, np.float64(5.0)),
        #
        # Build (geometry_dict, value) tuples the right way
        shapes = list(zip(
            [g.__geo_interface__ for g in gdf.geometry],
            elev_vals,
        ))
        #print("$$$$$$",type(shapes), shapes[:10])
        polygons = []
        for  g in gdf.geometry:
                polygons.append([g.__geo_interface__ ])

        from geocube.api.core import make_geocube
        gbis = gpd.GeoDataFrame(shapes,
                                geometry=polygons,
                                crs={"init": "epsg:31985"}
        )

        cube = make_geocube(vector_data=gbis)
        cube.data2.rio.to_raster("./output/test.tif")
...
def _elevation_from_raster(path: str) -> Optional[np.ndarray]:
    """
    Read band 1 of a GeoTIFF DEM and return a 2nd–98th percentile
    normalised float64 array.  Returns None on any error.
    """
    try:
        import rasterio
...
def _render_vector_layer(
    ...
    sq_bounds_3857: Tuple,
    size:           int,
    osm_fetch_fn:   Optional[callable] = None,
    layer_name:     str = "vector",
) -> Optional[Image.Image]:
    """
    Render a vector layer (roads, cadastral, hydrology) as RGBA overlay.
    If cfg.path is empty and osm_fetch_fn is provided, fetch from OSM.
    """
    try:
        import geopandas as gpd
    except ImportError:
    ...
    if cfg.path and Path(cfg.path).exists():
        try:
            ...
            print(f"  [{layer_name}] Could not load {cfg.path}: {exc}")
 
    if gdf is None and osm_fetch_fn is not None:
        try:
            gdf = osm_fetch_fn(sq_bounds_3857)
        except Exception as exc:
            print(f"  [{layer_name}] OSM fetch failed: {exc}")
    ...
    return _render_matplotlib_to_pil(_plot, sq_bounds_3857, size,
                                      bg_color="#00000000")
 
 
def _osm_roads_gdf(sq_bounds_3857: Tuple) -> Optional["gpd.GeoDataFrame"]:
    """Fetch OSM road network edges as GeoDataFrame (EPSG:3857)."""
    try:
        import osmnx as ox
        import geopandas as gpd
        from shapely.geometry import box
    except ImportError:
        raise ImportError("osmnx required for OSM roads.  pip install osmnx")
 
    sq4 = _bounds_3857_to_4326(sq_bounds_3857)

    poly = box(sq4[0], sq4[1], sq4[2], sq4[3])
    print("  [roads] Downloading OSM road network …")
    ...
    return edges.to_crs("EPSG:31985")
 
 
def _osm_hydrology_gdf(sq_bounds_3857: Tuple) -> Optional["gpd.GeoDataFrame"]:
    """Fetch OSM waterway features as GeoDataFrame (EPSG:3857)."""
    try:
        import osmnx as ox
        import geopandas as gpd
        from shapely.geometry import box
    except ImportError:
        raise ImportError("osmnx required for OSM hydrology.  pip install osmnx")
 
    sq4  = _bounds_3857_to_4326(sq_bounds_3857)
    poly = box(sq4[0], sq4[1], sq4[2], sq4[3])
    print("  [hydrology] Downloading OSM waterway features …")
...
class SatelliteAnnotator(tk.Tk):
    """
    ...
    def __init__(
        ...
        if pil_image is not None:
            ...
            bg, sq3, sq4, gdf = _load_source_with_meta(image_path, layer,
                                                         CANVAS_SIZE)
    ...
    def _load_layer(self, name: str, silent: bool = False):
        """Load (or fetch) a layer image and store in the renderer."""
                ...
                if not lcfg.path:
                    if not silent:
                        ...
                        if path:
                            ...
                            self._layer_status[name].set("Cancelled")
                            return
                img = _render_elevation_layer(lcfg, bounds, CANVAS_SIZE)
               # img = _render_vector_layer(lcfg, bounds, CANVAS_SIZE, layer_name="elevation")
                ...
                if not lcfg.path:
                    if not silent:
                        ...
                        if path:
                            ...
                            return
                img = _render_vector_layer(lcfg, bounds, CANVAS_SIZE,
                                            #osm_fetch_fn=_osm_hydrology_gdf,
                                            layer_name="hydrology")
 
            if img is not None:
                self._renderer.set_layer(name, img)
                self._renderer.set_layer_alpha(name,
                    self._layer_alpha_vars[name].get() / 100.0)
                self._renderer.set_layer_visible(
                    name, self._layer_vis_vars[name].get())
                self._layer_status[name].set("✓ Loaded")
            else:
    ...
    def _auto_annotate_nohabitat(self):
        ...
        (default 75 %) of their pixels are black in the background image.
...
# =============================================================================
# 9.  ROAD-BASED ACCESSIBILITY MATRIX
# =============================================================================
...
def compute_accessibility_from_roads(
    ...
    crs:            str   = "EPSG:4326",
    to_projected:   str   = "EPSG:3857",
) -> np.ndarray:
    """
    Compute binary N×N accessibility matrix from distance to OSM road nodes.
 
    Parameters
    ----------
    polygon        : shapely Polygon or GeoDataFrame  (study area in `crs`)
    N              : grid resolution
    max_distance_m : cells within this distance of a road node → accessible
    network_type   : OSM network type  ("all", "drive", "walk", …)
    crs            : input CRS of `polygon`
    to_projected   : metre-based CRS for distance calculation
 
    Returns
    -------
    ndarray (N, N) dtype int, values {0, 1}
 
    Dependencies: osmnx, geopandas, scipy
    """
    try:
        import osmnx as ox
        import geopandas as gpd
        ...
        raise ImportError(
            f"osmnx, geopandas, scipy required: pip install osmnx geopandas scipy\n"
            f"Detail: {exc}")
 
    if hasattr(polygon, "geometry"):
        poly_gdf = polygon.copy()
        if poly_gdf.crs is None:
            poly_gdf = poly_gdf.set_crs(crs)
        poly_gdf = poly_gdf.to_crs(crs)
        study_poly = unary_union(poly_gdf.geometry)
    else:
        study_poly = polygon
 
    print(f"  Downloading OSM network (type='{network_type}') …")
    try:
        G = ox.graph_from_polygon(study_poly, network_type=network_type,
                                   retain_all=True)
    except Exception as exc:
        warnings.warn(f"OSM download failed: {exc}. Returning all-accessible matrix.")
        return np.ones((N, N), dtype=int)
 
    if len(G.nodes) == 0:
        warnings.warn("No road nodes found. Returning all-inaccessible matrix.")
        return np.zeros((N, N), dtype=int)
 
    nodes, _ = ox.graph_to_gdfs(G)
    nodes_proj = nodes.to_crs(to_projected)
    node_xy    = np.column_stack([nodes_proj.geometry.x, nodes_proj.geometry.y])
 
    poly_proj = gpd.GeoDataFrame(geometry=[study_poly], crs=crs).to_crs(to_projected)
    minx, miny, maxx, maxy = poly_proj.total_bounds
    cw = (maxx - minx) / N; ch = (maxy - miny) / N
    ci = np.arange(N) + 0.5; ri = np.arange(N) + 0.5
    cx, cy = np.meshgrid(minx + ci * cw, miny + ri * ch)
    cents  = np.column_stack([cx.ravel(), cy.ravel()])
 
    print(f"  KD-Tree: {len(cents)} centroids vs {len(node_xy)} nodes …")
    dists, _ = cKDTree(node_xy).query(cents, k=1, workers=-1)
    accessible = (dists.reshape(N, N) <= max_distance_m).astype(int)
    print(f"  Accessible: {accessible.sum()}/{N*N} cells ≤ {max_distance_m:.0f} m")
    return accessible
...
def annotate_satellite(
    ...
    """Launch interactive annotation for a satellite image (PNG/JPEG/GeoTIFF)."""
    ...
    app = SatelliteAnnotator(image_path=image_path, N=N, cfg=cfg)
    app.mainloop()
...
def annotate_gpkg(
    ...
    print(f"Rasterising {gpkg_path} (layer: {layer}) …")
    pil_bg, sq3, sq4, gdf = _load_gpkg_with_meta(gpkg_path, layer, CANVAS_SIZE)
 
    app = SatelliteAnnotator(
        pil_image=pil_bg, N=N, source_gdf=gdf,
        source_label=Path(gpkg_path).name,
        sq_bounds_3857=sq3, sq_bounds_4326=sq4, cfg=cfg)
    app.mainloop()
...
def _run_from_config(cfg: AnnotatorConfig):
    ...
    if d.gpkg_path:
        ...
        annotate_satellite(image_path=d.image_path, N=d.N, ask_N=False, cfg=cfg)
    else:
        annotate_satellite(N=d.N, ask_N=True, cfg=cfg)
 
 
def main():
    parser = argparse.ArgumentParser(
        description="satellite_annotator.py — interactive annotation for FOREMOST",
    ...
    parser.add_argument("--no-ask-N", action="store_true")
    parser.add_argument("--write-config", action="store_true",
                        help="Write conf/annotator.yaml and exit")
    ...
    if args.gpkg:
        gdf = annotate_gpkg(gpkg_path=args.gpkg, layer=args.layer,
                             N=args.N, ask_N=not args.no_ask_N, cfg=cfg)
        if gdf is not None:
        ...
        arrays = annotate_satellite(image_path=args.image or None,
                                     N=args.N, ask_N=not args.no_ask_N, cfg=cfg)
        if arrays:
            print("\nExported arrays:")
            for name, arr in arrays.items():
