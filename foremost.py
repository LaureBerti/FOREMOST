...
from annotation_v9 import annotate_satellite
...
def write_default_yaml(config_dir: Union[str, Path] = "conf") -> Path:
    ...
    data:
      ...
      locked_out_path:  ""
      elevation_path:   ""   # optional DEM raster
...
def _cfg_to_dataclass(cfg) -> ForemostConfig:
    ...
    def _sub(d, key, cls):
        ...
        if isinstance(sub, cls):
            return sub
        kw = dict(sub) if hasattr(sub, '__iter__') and not isinstance(sub, str) \
...
class HabitatData:
    ...
    def synthetic(
       ...
       # print(np.sum(np.sum(restorable)))
        ...
        accessible_temp = np.random.choice([0, 1], size=(nrows, ncols),
                                           p=[1. - accessible_fraction, accessible_fraction])
        ...
        return cls(
            habitat=habitat, restorable=restorable,
            accessible=accessible, cost=cost,
            cell_area=cell_area, locked_out=locked_out,
            elevation=elevation, bg_image=bg,
        )
    ...
    def from_rasters(
        ...
        locked_out = (_load(locked_out_path) == 1).astype(bool) \
            if locked_out_path else None
        elevation  = _load(elevation_path) if elevation_path else None
 
        if aggregation_factor > 1:
            habitat, restorable, accessible, locked_out, cost, elevation = \
                _aggregate(habitat, restorable, accessible, locked_out, cost,
                           aggregation_factor, habitat_threshold, nodata_value,
                           elevation)
        ...
        if bg_image_path:
            try:
                ...
                pil = pil.resize((habitat.shape[1], habitat.shape[0]),
                                  PILImage.LANCZOS)
                bg = np.array(pil)
        ...
        return cls(
            habitat=habitat.astype(int), restorable=restorable,
            accessible=accessible, cost=np.clip(cost, 0, None),
            cell_area=cell_area, locked_out=locked_out,
    ...
    def from_config(cls, cfg: ForemostConfig) -> "HabitatData":
        ...
        """
        d = cfg.data
        has_paths = all(bool(getattr(d, k))
                        for k in ("habitat_path", "restorable_path",
                                  "accessible_path"))
        if has_paths:
            ...
            data = cls.synthetic(
                nrows=d.nrows, ncols=d.ncols,
                habitat_fraction=d.habitat_fraction,
                cell_area=d.cell_area, seed=d.seed,
            )
...
def _aggregate(habitat, restorable, accessible, locked_out, cost,
               factor, threshold, nodata_value, elevation=None):
    ...
    for i in range(nr):
        for j in range(nc):
            ...
            bh = habitat[sl]; valid = bh != nodata_value
            if not valid.any():
                continue
    ...
    return (h_a, r_a, a_a,
            (lo_a if locked_out is not None else None),
            c_a, e_a)
...
def mesh(habitat_grid: np.ndarray, total_cells: int,
...
def iic(habitat_grid: np.ndarray, total_cells: int,
        cell_area: float = 1.0, max_dist: int = 10) -> float:
...
def diameter(sel_grid: np.ndarray) -> int:
    ...
    if len(cells) == 1: return 1
    cell_set = set(cells)
 
    def bfs(start):
        dist = {start: 0}; q = [start]; mx = 0; far = start
        while q:
            nq = []
            for r, c in q:
                for dr in (-1, 0, 1):
                    for dc in (-1, 0, 1):
                        ...
                        if nb in cell_set and nb not in dist:
                            dist[nb] = dist[(r, c)] + 1
                            if dist[nb] > mx: mx, far = dist[nb], nb
                            nq.append(nb)
            q = nq
        return mx, far
...
class ObjectiveType(str, Enum):
    ...
    def n_obj(self) -> int:
        return {
            self.MESH: 1, self.IIC: 1, self.COST: 1,
            self.MESH_IIC: 2, self.MESH_COST: 2, self.IIC_COST: 2,
            self.FULL: 3,
        }[self]
...
class RestorationProblem(ElementwiseProblem):
    def __init__(
        ...
        super().__init__(
            n_var=self.n_candidates, n_obj=objective.n_obj,
    ...
    def _violation(self, x):
        ...
        sel = x.astype(bool)
        v   = 0.0
        area = float(self.habitat_data.restorable[
            self._candidate_rows[sel], self._candidate_cols[sel]].sum())
        if area < c.min_restore: v += (c.min_restore - area) ** 2
        if area > c.max_restore: v += (area - c.max_restore) ** 2
        tc = float(self.habitat_data.cost[
            self._candidate_rows[sel], self._candidate_cols[sel]].sum())
        if tc > c.max_cost:
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        if obj.needs_iic:
            iic_v = iic(hg, self.total_area_cells,
                        self.habitat_data.cell_area, self.iic_max_dist)
        if obj.needs_cost:
            cost_v = float(self.habitat_data.cost[
                self._candidate_rows[sel], self._candidate_cols[sel]].sum())
...
def _build_algorithm(
    ...
    common = dict(pop_size=pop_size, sampling=sampling,
    ...
    if name == "CTAEA":
        if not _HAS_CTAEA:
            warnings.warn("CTAEA not available (pymoo ≥ 0.6 required); "
                          "falling back to NSGA2.")
            return NSGA2(**common)
        if n_obj < 2:
            warnings.warn("CTAEA is multi-objective; using NSGA2 for single-obj.")
            return NSGA2(**common)
        n_part = max(4, pop_size // (n_obj * 3))
        ref_dirs = get_reference_directions("das-dennis", n_obj,
                                             n_partitions=n_part)
        # CTAEA needs ref_dirs and does NOT accept eliminate_duplicates
        common_ctaea = {k: v for k, v in common.items()
                        if k != "eliminate_duplicates"}
    ...
    if name == "RNSGA3":
        if not _HAS_RNSGA3:
            ...
            return NSGA2(**common)
        if n_obj < 2:
            warnings.warn("RNSGA3 is multi-objective; using NSGA2 for single-obj.")
            return NSGA2(**common)
        ...
        return RNSGA3(ref_points=ref_points, mu=0.1, pop_per_ref_point = 55)
 
    raise ValueError(
        f"Unknown algorithm '{algo_name}'. "
        f"Choose from: {SUPPORTED_ALGOS}"
    )
...
def solve(
    ...
    """
    # Auto-select algorithm for single-obj
    if not problem.objective.is_multi and algo_name.upper() not in ("GA", "NSGA2"):
        warnings.warn(f"{algo_name} is multi-objective; switching to GA for "
    ...
    algorithm = _build_algorithm(
        algo_name, problem.objective.n_obj,
        pop_size, sampling, crossover, mutation,
    )
    ...
    if verbose:
        w = 62
        print(f"\n{'─'*w}")
        print(f"  FOREMOST  |  algo : {actual_algo:<10} "
              f"|  obj : {problem.objective.value}")
        print(f"  candidates : {problem.n_candidates:>4d}  "
    ...
    result = minimize(
        ...
        callback=ProgressCallback(every=max(1, n_gen // 10)) if verbose else None,
    )

    if problem.objective.is_multi:
        solutions = [problem.decode_solution(result.X[i])
    ...
    if verbose:
        ...
        if not problem.objective.is_multi:
            ...
            print(f"  Diameter        : {s['diameter_cells']} cell(s)")
            print(f"  # conn. comp.   : {s['n_connected_components']}")
        else:
            print(f"  Pareto front    : {len(solutions)} non-dominated solution(s)")

    return dict(result=result, solutions=solutions, algo_name=actual_algo)
...
def _overlay_bg(ax, bg_image: Optional[np.ndarray], alpha: float = 0.35):
    ...
    if img.ndim == 2:
        ...
        ax.imshow(img, cmap="gray", aspect="auto",
                  vmin=0, vmax=1, alpha=alpha, zorder=0, interpolation="nearest")
    else:
        # RGB — normalize if needed
        if img.max() > 1.0:
            img = img / 255.0
        img = np.clip(img, 0, 1)
        ax.imshow(img, aspect="auto", alpha=alpha, zorder=0,
...
def plot_solution(
    ...
    fig.suptitle(full_title, fontsize=13, fontweight="bold", y=0.99,
                 color="#111111")
    ...
    ax.imshow(data.habitat, cmap=hc, norm=hn, interpolation="nearest",
              alpha=0.85, zorder=1)
    ax.set_title(f"Original habitat  {algo_tag}", fontsize=10,
                 fontweight="bold", pad=5)
    _hab_legend(ax); ax.axis("off")

    # ── (0,1) post-restoration ────────────────────────────────────────────────
    ax = fig.add_subplot(gs[0, 1])
    _overlay_bg(ax, bg, alpha=bg_alpha)
    ax.imshow(combined, cmap=hc, norm=hn, interpolation="nearest",
              alpha=0.85, zorder=1)
    ax.set_title(f"Post-restoration  {algo_tag}", fontsize=10,
                 fontweight="bold", pad=5)
    _hab_legend(ax); ax.axis("off")
    ...
    im = ax.imshow(cost_disp, cmap="YlOrRd", interpolation="nearest",
                   alpha=0.85, zorder=1)
    ...
    ax.set_title(f"Restoration cost  {algo_tag}", fontsize=10,
                 fontweight="bold", pad=5)
    ax.axis("off")
    ...
    ax.imshow(solution["selection_grid"], cmap=sc_cm, norm=sc_nm,
              interpolation="nearest", alpha=0.85, zorder=1)
    ax.set_title(f"Selected cells  {algo_tag}", fontsize=10,
                 fontweight="bold", pad=5)
    patches = [mpatches.Patch(color=_C["restored"], label="Selected"),
    ...
    im = ax.imshow(rest_disp, cmap=hr, interpolation="nearest",
                   alpha=0.85, zorder=1)
    if solution["selection_grid"].sum() > 0:
        ax.contour(solution["selection_grid"], zorder=2, **sel_contour_kw)
    #plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04, label="Restorable area")
    ax.set_title(f"Restorable area \n(blue = selected)  {algo_tag}", fontsize=10,
                 fontweight="bold", pad=5)
    ax.axis("off")
    ...
    for yi, (label, fmt, color) in zip(y_pos, metrics):
        ax.text(0.03, yi, label, va="center", fontsize=8,
                color="#444444", transform=ax.transAxes)
        ax.text(0.97, yi, fmt, va="center", ha="right", fontsize=8,
                fontweight="bold", color=color, transform=ax.transAxes)
    for yi in y_pos[:-1]:
        ax.axhline(yi - (y_pos[0] - y_pos[1]) * 0.4,
                   color="#dddddd", lw=0.7, xmin=0.01, xmax=0.99)
    ax.set_title(f"Solution metrics  {algo_tag}", fontsize=10,
                 fontweight="bold", pad=5)
...
def plot_pareto_front(
    ...
    def _val(s, label):
        if label == "MESH":       return s["mesh"]
        if label == "IIC":        return s["iic"]
        if label == "Total cost": return s["total_cost"]
        return 0.0
    ...
    sc = ax.scatter(xv, yv, c=costs, cmap="YlOrRd",
                    s=[25 + 7 * n for n in n_cells],
                    edgecolors="#555", linewidths=0.4, alpha=0.85, zorder=3)
    ...
    for idx, lbl in [
        ...
        ax.annotate(lbl, xy=(xv[idx], yv[idx]),
                    xytext=(12, 12), textcoords="offset points",
                    fontsize=8, color=_C["accent"],
...
def plot_pareto_front_3d(
    ...
    sc  = ax.scatter(mesh_v, iic_v, cost_v,
                     c=cost_v, cmap="YlOrRd",
                     s=40, edgecolors="#444", linewidths=0.3, alpha=0.8)
...
def plot_solutions_comparison(
    ...
    n     = len(solutions)
    fig_w = max(14, 4 * n)
    fig, axes = plt.subplots(2, n, figsize=figsize or (fig_w, 8),
                             facecolor=_C["bg"])
    if n == 1:
    ...
    for col, (sol, lab) in enumerate(zip(solutions, labels)):
        ...
        at.imshow(sol["selection_grid"], cmap=sc, norm=sn,
                  interpolation="nearest", alpha=0.85, zorder=1)
        at.contour(data.habitat_mask.astype(int), levels=[0.5],
                   colors=[_C["habitat"]], linewidths=0.8, alpha=0.6, zorder=2)
        ...
        for yi, (mlabel, mval, mcolor) in zip(y_pos, metric_rows):
            ...
            ab.barh(yi, norm_v, height=0.10, color=mcolor,
                    alpha=0.75, transform=ab.transAxes)
            ab.text(-0.02, yi, mlabel, va="center", ha="right",
                    fontsize=7.5, transform=ab.transAxes, color="#333")
            ab.text(norm_v + 0.02, yi, f"{mval:.3f}", va="center",
                    ha="left", fontsize=7.5, fontweight="bold",
                    color=mcolor, transform=ab.transAxes)
        ab.set_xlim(0, 1.45)
        if col == 0:
...
def plot_cost_surface(
    ...
    fig.suptitle(f"Ecological cost function components{algo_tag}",
    ...
    if data.elevation is not None:
        ...
        im = ax.imshow(elev_disp, cmap="terrain",
        ...
        im = ax.imshow(acc_disp, cmap="RdYlGn",
                       interpolation="nearest", alpha=0.9, zorder=1)
    ...
    rest_disp[data.habitat == 1] = np.nan
    rest_disp[data.habitat == data.nodata_value] = np.nan
    im = ax.imshow(rest_disp, cmap="YlGn",
                   interpolation="nearest", alpha=0.9, zorder=1)
    plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04, label="Restorable area")
...
class ForemostProblemBuilder:
    ...
    def set_max_mesh_objective(self)  -> "ForemostProblemBuilder":
        self._objective = ObjectiveType.MESH;      return self
    def set_max_iic_objective(self)   -> "ForemostProblemBuilder":
        self._objective = ObjectiveType.IIC;       return self
    def set_min_cost_objective(self)  -> "ForemostProblemBuilder":
        self._objective = ObjectiveType.COST;      return self
    def set_mesh_iic_objective(self)  -> "ForemostProblemBuilder":
        self._objective = ObjectiveType.MESH_IIC;  return self
    def set_mesh_cost_objective(self) -> "ForemostProblemBuilder":
        self._objective = ObjectiveType.MESH_COST; return self
    def set_iic_cost_objective(self)  -> "ForemostProblemBuilder":
        self._objective = ObjectiveType.IIC_COST;  return self
    def set_full_objective(self)      -> "ForemostProblemBuilder":
        self._objective = ObjectiveType.FULL;      return self

    # ── constraints ───────────────────────────────────────────────────────────
    def add_restorable_constraint(self, min_restore: float = 20.0,
    ...
    def add_compactness_constraint(self, max_diameter: int = 5
                                   ) -> "ForemostProblemBuilder":
        self._constraints.max_diameter = max_diameter; return self

    def add_connected_constraint(self, max_nb_cc: int = 1
                                  ) -> "ForemostProblemBuilder":
        self._constraints.max_nb_cc = max_nb_cc; return self

    def add_locked_out_constraint(self) -> "ForemostProblemBuilder":
        return self   # embedded in HabitatData.candidate_mask

    def add_min_proportion_constraint(self, min_proportion: float
                                       ) -> "ForemostProblemBuilder":
        self._constraints.min_proportion = min_proportion; return self

    def add_budget_constraint(self, max_cost: float) -> "ForemostProblemBuilder":
        self._constraints.max_cost = max_cost; return self

    def set_penalty(self, p: float)   -> "ForemostProblemBuilder":
        self._penalty = p; return self
    def set_iic_max_dist(self, d: int) -> "ForemostProblemBuilder":
        self._iic_max_dist = d; return self

    # ── build / solve ─────────────────────────────────────────────────────────
    def build(self) -> RestorationProblem:
        return RestorationProblem(
            data=self._data, constraints=self._constraints,
            objective=self._objective, penalty=self._penalty,
    ...
    def solve(
        ...
        return solve(self.build(), pop_size=pop_size, n_gen=n_gen,
                     seed=seed, verbose=verbose, algo_name=algo)


# =============================================================================
# 8.  DEMO / HYDRA ENTRY POINT
# =============================================================================

def run_demo(cfg: Optional[ForemostConfig] = None):
    """
    End-to-end demonstration on a synthetic 30×30 landscape.
    Runs all 7 objective types with configurable algorithms.
    Now uses the ecological cost function and shows satellite background.

    Parameters
    ----------
    cfg : ForemostConfig dataclass.  If None, uses defaults.
    """
    if cfg is None:
        cfg = ForemostConfig()

    print("\n" + "=" * 65)
    print("  FOREMOST  —  Ecological restoration planning")
    print(f"  Algorithm : {cfg.optimizer.algo} | "
          f"Objective : {cfg.optimizer.objective}")
    print("=" * 65)

    # ── build landscape ───────────────────────────────────────────────────────
    data = HabitatData.from_config(cfg)
    print(f"\n  Habitat cells     : {data.habitat_mask.sum()}")
    print(f"  Candidate cells   : {data.candidate_mask.sum()}")
    print(f"  Cost range (€)    : "
          f"{data.cost[data.candidate_mask].min():.1f} – "
          f"{data.cost[data.candidate_mask].max():.1f}")
    print(f"  Cost total (€)    : {data.cost[data.candidate_mask].sum():.0f}")
    ...
    max_diameter = cfg.constraints.max_diameter
    max_nb_cc = cfg.constraints.max_nb_cc
    max_cost = cfg.constraints.max_cost
    ...
    show   = cfg.output.show
    dpi    = cfg.output.dpi
    p_ref_point = cfg.optimizer.pop_per_ref_point
    mu  = cfg.optimizer.mu

    # ── 1. Maximise MESH ─────────────────────────────────────────────────────
    print(f"\n--- 1/7  Maximise MESH  [{algo}] ---")
    r_mesh = (ForemostProblemBuilder(data).set_max_mesh_objective()
              ...
              .solve(pop_size=ps, n_gen=ng, seed=sd, verbose=vb, algo=algo))
    plot_solution(data, r_mesh["solutions"][0],
                  title="Single-obj — Maximise MESH",
                  algo_name=r_mesh["algo_name"],
                  save_path=f"{out_dir}/{pfx}_{algo}_1_mesh.png",
                  show=show)

    # ── 2. Maximise IIC ──────────────────────────────────────────────────────
    print(f"\n--- 2/7  Maximise IIC  [{algo}] ---")
    r_iic = (ForemostProblemBuilder(data).set_max_iic_objective()
             .add_restorable_constraint(min_restore=min_restore, max_restore=max_restore)
             .add_compactness_constraint(max_diameter=max_diameter)
             .add_connected_constraint(max_nb_cc=max_nb_cc)
             .add_budget_constraint(max_cost=max_cost)
             .solve(pop_size=ps, n_gen=ng, seed=sd, verbose=vb, algo="GA"))
    plot_solution(data, r_iic["solutions"][0],
                  title="Single-obj — Maximise IIC",
                  algo_name=r_iic["algo_name"],
                  save_path=f"{out_dir}/{pfx}_{algo}_2_iic.png",
                  show=show)
    ...
    # ── 5. Pareto: MESH × Cost ────────────────────────────────────────────────
    print(f"\n--- 5/7  Pareto: MESH × Cost  [{algo}] ---")
    r_mc = (ForemostProblemBuilder(data).set_mesh_cost_objective()
            .add_restorable_constraint(min_restore=min_restore, max_restore=max_restore)
            .add_compactness_constraint(max_diameter=max_diameter)
            .add_connected_constraint(max_nb_cc=max_nb_cc)
            .add_budget_constraint(max_cost=max_cost)
            .solve(pop_size=ps, n_gen=ng, seed=sd, verbose=vb, algo=algo))
    plot_pareto_front(r_mc["solutions"], ObjectiveType.MESH_COST,
                      algo_name=r_mc["algo_name"],
                      save_path=f"{out_dir}/{pfx}_{algo}_5_pareto_mesh_cost.png",
                      show=show)

    # ── 6. Pareto: IIC × Cost ─────────────────────────────────────────────────
    print(f"\n--- 6/7  Pareto: IIC × Cost  [{algo}] ---")
    r_ic = (ForemostProblemBuilder(data).set_iic_cost_objective()
            .add_restorable_constraint(min_restore=min_restore, max_restore=max_restore)
            .add_compactness_constraint(max_diameter=max_diameter)
            .add_connected_constraint(max_nb_cc=max_nb_cc)
            .add_budget_constraint(max_cost=max_cost)
            .solve(pop_size=ps, n_gen=ng, seed=sd, verbose=vb, algo=algo))
    plot_pareto_front(r_ic["solutions"], ObjectiveType.IIC_COST,
                      algo_name=r_ic["algo_name"],
                      save_path=f"{out_dir}/{pfx}_{algo}_6_pareto_iic_cost.png",
                      show=show)

    # ── 7. Pareto FULL: MESH × IIC × Cost ────────────────────────────────────
    print(f"\n--- 7/7  Pareto FULL: MESH × IIC × Cost  [{algo}] ---")
    r_full = (ForemostProblemBuilder(data).set_full_objective()
              .add_restorable_constraint(min_restore=min_restore, max_restore=max_restore)
              .add_compactness_constraint(max_diameter=max_diameter)
              .add_connected_constraint(max_nb_cc=max_nb_cc)
              .add_budget_constraint(max_cost=max_cost)
              .solve(pop_size=ps, n_gen=ng, seed=sd, verbose=vb, algo=algo))
    plot_pareto_front_3d(r_full["solutions"],
                         algo_name=r_full["algo_name"],
                         save_path=f"{out_dir}/{pfx}_{algo}_7_pareto_3d.png",
                         show=show)
...
def main():
    ...
    • If hydra-core is installed:  delegates to @hydra.main (YAML config + CLI overrides)
    • If --write-config is passed:  writes conf/foremost.yaml and exits
    • Otherwise:                    runs demo with default ForemostConfig()
    """
    import argparse
    ...
    parser.add_argument("--write-config", action="store_true",
                        help="Write default conf/foremost.yaml and exit")
    parser.add_argument("--algo",      type=str, default=None,
                        choices=list(SUPPORTED_ALGOS),
                        help="Override optimizer algorithm")
    parser.add_argument("--objective", type=str, default=None,
                        choices=[o.value for o in ObjectiveType],
                        help="Override objective type")
    ...
    # ── no Hydra: build config from CLI args + defaults ───────────────────────
    cfg = ForemostConfig()
    if args.algo:      cfg.optimizer.algo      = args.algo
    if args.objective: cfg.optimizer.objective = args.objective
    if args.pop_size:  cfg.optimizer.pop_size  = args.pop_size
    if args.n_gen:     cfg.optimizer.n_gen     = args.n_gen
    if args.seed:      cfg.optimizer.seed      = args.seed
    if args.out_dir:   cfg.output.dir          = args.out_dir
